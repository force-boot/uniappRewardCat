		...
		<view class="u-datetime-picker" @tap.stop>
			<view class="u-picker-header" @touchmove.stop.prevent="stop" catchtouchmove="stop">
				<view class="u-btn-picker u-btn-picker--tips" :style="{ color: cancelColor }" hover-class="u-opacity" :hover-stay-time="150"
				 ...
				 <view class="flex align-center justify-center" v-show="zdyClick" @click="zdyOn()"><u-checkbox-group>
					<u-checkbox @change="zdyOn()" icon-size="13" active-color="#f8c750" shape="circle" v-model="checkedTime"></u-checkbox>
				</u-checkbox-group><text style="font-size: 28rpx;" class="ml-1">数量完成自动结束</text></view>
...
<script>
	...
	export default {
		...
		methods: {
			zdyOn(){
				this.checkedTime = !this.checkedTime;
				return this.$emit('zdyOn',this.checkedTime);
			},
